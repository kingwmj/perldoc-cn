...
PDF created with pdfFactory Pro trial version www.pdffactory.com载入 IO::Handle 类是一项代价高昂的操作，因此你该知道如何使用常规的内建变量。 
...
下面是代码失控的一个例子： 
    ...
    sub nasty_break { 
        $_ = 5; 
PDF created with pdfFactory Pro trial version www.pdffactory.com        # do something with $_ 
    } 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com将其设为非零整数值就可以进行字符串内的多行匹配，设为 0(或未定义值) 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com=item $| 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com=item $SUBSCRIPT_SEPARATOR 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com在对某个变量 $var 进行匹配后： 
...
默认为 S<" \n-">，使字符串能在空白或连字符处断开。 
PDF created with pdfFactory Pro trial version www.pdffactory.com(助记：诗中的“冒号”是一行的一部分。) 
...
(助记：语法错误发生“在”哪里？) 
PDF created with pdfFactory Pro trial version www.pdffactory.com(译注：符号“@”为英文单词“at”简写，意为“在……”) 
...
PDF created with pdfFactory Pro trial version www.pdffactory.comC<< $< >>、C<< $> >>、C<$(> 和 C<$)> 只能在支持对应的 I<set[re][ug]id()> 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com=item $^H 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com=item $INPLACE_EDIT 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com通常可以用 $^X 的值再次产生和当前运行的 perl 相同的一个独立副本，例如， 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com是第一个参数，而I<不是>程序本身的命令名称。命令名称请见 C<$0>。 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com请确保没有使用裸字作为一个信号处理器的名字，免得产生无意中的调用。 
...
PDF created with pdfFactory Pro trial version www.pdffactory.com         To see backtrace try starting Perl with -MCarp switch"; 
...
遇到的特定错误 
PDF created with pdfFactory Pro trial version www.pdffactory.com条件(程序的 exit() 值)，低 8 位反映失败方式，例如信号致死或核心转储，细节参见 
wait(2)。对比 
