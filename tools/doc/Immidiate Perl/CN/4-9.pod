...
运用书中最后那个版本的check_required_items，写一个子程序check_items_for_all,把一个散列引用作为惟一参数。这个散列引用的键是在Minnow船上的所有乘客，其对应的值是他们各自想带上船的东西。
...
我们新建的那个子程序要调用check_required_items来为散列中每个人更新其必需的装备清单。
