...
=EN If you look at @skipper, you’ll see that it consists of two parts: the @ symbol and the name of the array.
=CN 我们看一下 @skipper, 你会发现它包括两部份：@符号和数组名。
=EN Similarly, the syntax $skipper[1] consists of the name of the array in the middle and some syntax around the outside to get at the second element of the array
=CN 相似地，语法 $skipper[1] 包括当中的数组名和围绕在周围的语法符号表示取这个数组的第二个元素
=EN index value 1 is the second element because index values start at 0
=CN 索引1表示取第二个元素，因为索引起始值是0
...
=CN 这里有一个小戏法：我们可以用在外面套上大括号的指向数组的引用，来替换数组的名字，其结果就是访问原始的数组。
=EN That is, wherever we write skipper to name the array, we use the reference inside curly braces: { $items }.
=CN 换句话说，就是我们写sipper数组名字的地方，可以用大括号包起来的指向数组的引用来代替：{$items}。
...
=CN 以上每个例子中，$items指向一个不同的数组。如此，同样的代码每次调用的时候可以应用到不同的数组。
...
=CN 即，相比以前我们拷贝供应清单到 @_ 数组，现在我们传一个指向供应清单数组的单值。
...
=CN The second element is a reference to the correct provisions array, which we use in the grep expression.
=EN 第一个元素是成员或乘务员的名字，我们用它来组成出错信息。
=CN 第二个元素是指向供应清单数组的引用。我们把它用在grep表达式中。
