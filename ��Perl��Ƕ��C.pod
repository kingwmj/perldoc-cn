...
你不是唯一一个在编译本文档的例子时遇到困难的。一个重要规则是：用编译你
的 
...
C 程序中使用 Perl——这也是为什么你不应该从另一台机器中复制 Perl 的可
执 
行程序而不复制 I<lib> 目录。） 
...
（就这一行。）在我的 DEC  Alpha 使用旧的 5.003_05，这个“咒语”有一点不
同： 
...
很简单。现在编译并运行： 
    ...
    % cc -o showtime showtime.c `perl -MExtUtils::Embed -e ccopts -e 
ldopts` 
...
所有在名字中含有 I<sv> 的奇怪函数都是为了协助将 Perl 标量转换为 C 类
型。 
 ...
 ** 
 ** Used for =~ operations that modify their left-hand side (s/// and 
tr///) 
 ** 
...
它产生这样的输出（过长的行再次折叠了）： 
   ...
   qrtr, bt h hs n d *wht*.  H fmbls thrgh hs rd sqzy chngprs nd gvs th 
by 
   thr xtr pnns wth hs dllr, hpng tht h mght lck nt th crrct mnt.  Th by 
gvs 
...
这是我们的要做的：C 程序放置一些参数到“Perl 栈”中，当魔术发生时闭上
它的 
...
辑判断来跟踪进程的大小，或者在一定次数的请求之后重新启动一次，这样来保
证内 
存的消耗是保证最小的。你可能还会在可能的时候用 L<perlfunc/my> 限定变量
的范围。 
...
Now compile: 
 ...
 % cc -o persistent persistent.c `perl -MExtUtils::Embed -e ccopts -e 
ldopts` 
...
选项来编译 perl。 否则一些解释器的变量在连续运行过程中不会正确的初始化，
你 
的运行程序可能会崩溃。 
 
如果你打算在不同线程中并发运行多个解释器时，使用 C<-Dusethreads 
-Duseithreads> 而不是C<-Dusemultiplicity> 可能更合适。因为这可以对解
释 
器支持链接到系统的线程库。 
...
像通常那样编译： 
 ...
 % cc -o multiplicity multiplicity.c `perl -MExtUtils::Embed -e ccopts 
-e ldopts` 
...
在编译标签中加入 -DPERL_NO_SHORT_NAMES，你就可以隐藏 Perl 公共接口的简
短 
形式。这意味着你不能这样写： 
...
（参考 L<perlguts/Background and PERL_IMPLICIT_CONTEXT for the 
explanation 
...
有时可以在 C 中写出 I<运行更快的代码（write  faster  code）>，但是你总是
可以在 
Perl 中I<更快地写出代码（write  code  faster）>。因为你可以相互使用对方，
只 
要你需要可以结合起来。 
