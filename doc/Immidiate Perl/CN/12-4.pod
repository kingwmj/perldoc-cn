...
我们调用构建函数named来快速地把参数作为马的名字。 我们可以用不同名字的构建函数来给对象加上生日（如谱系记录或生日）。 然而，大多数人喜欢用new来命名构建函数，并对不同的参数以不同的解释。 只要能太到目的，两种方法都可以。 CPAN上大多数模块用new， 也有些是例外，比如DBI模块的DBI->connect()。 这根据开发者的习俗。
