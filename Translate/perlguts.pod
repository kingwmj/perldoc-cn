...
only argument C<p>.
 ...
 C<SAVEDESTRUCTOR_X(DESTRUCTORFUNC_t f, void *p)>
...
=over 4
 ...
 C<SV* save_scalar(GV *gv)>

Equivalent to Perl code C<local $gv>.

 C<AV* save_ary(GV *gv)>

 C<HV* save_hash(GV *gv)>
...
=over 4
 ...
 C<void bhk_start(pTHX_ int full)>
...
double pointer so you can replace the OP if you need to.
 ...
 C<void bhk_post_end(pTHX_ OP **o)>
...
calls string eval.
 ...
 C<void bhk_eval(pTHX_ OP *const o)>
...
didn't have a matching C<start>.
 ...
 Examining internal data structures with the C<dump> functions
