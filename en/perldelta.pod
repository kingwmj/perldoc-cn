...
On Linux the legacy process name is now set with zip(2), in
...
On some platforms with unusual header files, like Win32 gcc(1) using C<mingw64>
...
packages using L<warnings::register>.  See perllexwarn(1) for more information.
...
split mostly from the NOTES section of the pod2man(1) manpage.
