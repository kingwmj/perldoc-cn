=EN 7.5. Returning a Subroutine from a Subroutine
=CN 7.5. 从子程序中返回一个子程序
...
=EN Returning the two subroutine references from the creating subroutine does not invoke them. The references are just data at that point. It’s not until we invoke them as a callback or an explicit subroutine dereferencing that they actually do their duty.
