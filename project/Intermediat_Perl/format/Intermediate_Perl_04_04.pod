...
=CN 一般来说, 还原对数组的引用大多是一个简单的标量变量, 比如：@{$items} 或者 ${$items}[1]. 在那些情况下, 我们可以把大括号去掉, @$items或$$items[1]这样的形式并不会引起歧义. 
...
=CN 但是, 有一点, 如果大括号里的内容不是简单的标量变量的话, 我们就不能把大括号去掉. 比如, 对于前面最后一个改写过的子例程中的@{$_[1]}, 我们不能把大括号去掉. 因为那是个正访问数组的元素, 而不是一个简单的标量变量. 
...
=CN 这个规则也方便我们知道哪里丢了大括号. 比如我们看到$$items[1]的时候, 知道这会有些语法上的麻烦, 我们会意识到必须在简单标量变量$items周围加上大括号. 如此, $items必须是一个指向数组的引用. 
