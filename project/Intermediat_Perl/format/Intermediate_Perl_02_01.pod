...
=CN 你可能已经知道Perl的一些列表操作符, 但并没有想过他们是怎么同列表一起工作的. 最常用的列表操作符应该是print了. 
...
=CN 当然，所有能用map和grep做的，都能用 foreach 循环语句实现。但其次，我们可以在前端继续优化代码。
...
=CN 重点是正确的应用 grep 和 map 能降低程序的复杂程度。让我们可以更关注更高级的问题而不是拘泥与细节。
