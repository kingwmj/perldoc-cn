=EN 5. 8. Autovivification and Hashes
=CN 5. 8. 自生成功能与散列
...
…
=EN Now the Professor wants to produce a summary of the source host, the destination host, and the total number of transferred bytes for the day. Tabulating the data is as simple as:
=CN 现在, 教授要生成一个有关源主机, 目标主机和总传输字节数的报表. 把数据列表显示出来：
my %total_bytes;
while (<>) {
