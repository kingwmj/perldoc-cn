...
Modules are the building 块s for our programs. They provide reusable 子程序, variables, and even object-oriented classes. On our way to building our own modules, we'll show you some of those you might be interested in. We'll also look at the basics of 使用模块 that others have already written.
...
当你在开发你的程序的时候, 你可能要考虑是否你应该仅仅用核心模块. 这样的话你就能保证任何用Perl的人都能执行你的代码, 只要他们的Perl版本同你相同.  [] 这里我们也不多费口舌了, 主要是因为我们太喜欢Perl综合典藏网了, 不用它就舍不得. 

=head2 3. 2. 使用模块

几乎所有的Perl模块都带有文档说明. 所以尽管我们可能不知道那些模块背后的戏法是怎么变的, 如果我们知道如何使用接口, 我们就不必去担心那些细节.  这就是在这里介绍接口的原因, 毕竟：它屏蔽了复杂性. 

在我们的本机当中, 我们可以用perldoc命令来调出模块文档.  We give it the module NAME we're interested in, and it prints out its 文档.
...
我们在这里列出了文档的一般结构（至少是最重要的部分）. Module 文档 typically follows the old Unix manpage format, which starts with a NAME and SYNOPSIS section.

The SYNOPSIS gives us 实例 of the module's use, and if we can suspend understanding for a bit and follow the example, we can use the module. 就是说, 这可以使你在还没有熟悉Perl技术和语法的情况下, 仅仅看这些例子, 就可以使程序工作起来. 
...
The list of names following use doesn't change which 子程序 are defined in the module's package (in this case, File::Basename). We can always use the full NAME regardless of the import list, as in:[*]
...
相比于File::Basename导出的子例程, 在核心模块中有另外一个File::Spec模块也提供类似的功能. File::Spec被设计来支持对文件属性的一般操作.  (A file specification is usually a file or directory name, but it may be a NAME of a file that doesn't existin which case, it's not really a filename, is it?)
...
如前所述, 这个模块没有导入任何东西.  Its entire interface uses class methods, such as new, against the class NAME to create instances, and then calls instance methods, such as bpow and bstr, against those instances.

=head2 3. 7. Perl综合典藏网模块仓库

Perl综合典藏网是众多志愿者协同工作的产物. 志愿者中的许多人用他们自己的FTP站点来维持前台的Perl综合典藏网 Web页面.  直到1993年底, 他们还是用perl-packrats邮件列表来协调他们的工作. 之后, 因为磁盘空间越来越便宜, 所以相同信息可以在所有的站点复制, 而不必放在专门的站点上.  这种想法酝酿了一年左右, 以Jarkko Hietaniemi(芬兰人, 详见：http://users. tkk. fi/jhi/jarkko. html. 中文名：沙雅可, 日文名：奴稗谷笑)在芬兰建立的FTP站点为母站, 其它的镜相站点可以以此来进行及时的更新. 

这个站点的一部份工作是重新编排和组织分离的Perl文档. 建立起放置为非UNIX系统的二进制文件、脚本、和Perl源代码的空间. 然而, Perl综合典藏网当然最关心的是占空间大部份的Perl模块部分. 

按模块的功能编目, Perl综合典藏网把模块用符号连接组织起来, 指向他们的作者目录－－实际文件所在的地方.  模块还包含以易于Perl分析的格式索引, 如Data::Dumper这样的输出来丰富模块索引的内容.  自然啦, 这一切编目索引都是有主服务器的Perl程序自动从数据库生成的.  一般来说, Perl综合典藏网中从一个服务器同步到另一个服务器的工作是由mirror. pl这个古老的Perl程序完成的. 

从屈指可数的几台镜相服务器开始, Perl综合典藏网如今已经成长为超过200公共服务器, 至少每天（有时是每小时）刷新一次的网络.  无论你在世界的哪头, 我们总是可以找到最近的Perl综合典藏网镜相站. 

Perl综合典藏网 Search (http://search. Perl综合典藏网. org)的难以置信的易用性, 一定会成为你最喜欢的搜寻界面.  从那个网页, 你可以搜寻模块、看它的文档、浏览它有哪些版本、查询他们的Perl综合典藏网测试者的报告以及许多其它事情. 

=head2 3. 8. 安装从Perl综合典藏网下载的模块

安装从Perl综合典藏网获得的简单模块可以很直接：先从Perl综合典藏网下载发布的文档, 解压到一个目录. 下例中我们用wget下载文档, 当然, 你可以用你习惯的工具下载. 
...
Because a use lib pragma will pretty much always have a site-dependent pathname, it is traditional and we encourage you to put it near the top of the file. This makes it easier to find and update when we need to move the file to a new system or when the lib directory's NAME 变更. (Of course, we can eliminate use lib entirely if we can install our modules in standard @INC locations, but that's not always practical. )
...
幸而, 我们有工具来助一臂之力. 自从Perl 5. 004版开始, Perl综合典藏网. pm模块成为核心发布的一部份. 它给我们提供了一个交互式的模块安装环境. 
...
To install a module along with its dependencies, we issue the install command with the NAME of the module. Now, Perl综合典藏网. pm handles all the work of downloading, unpacking, building, testing, and installing the module, and it does so recursively for all its dependencies.


 cpan> install CGI::Prototype 

如果觉得用上面的方法还烦琐, brian写了个Perl综合典藏网脚本放在Perl的发行包里. 我们只要简单的列出要安装的模块, 由脚本来处理余下的事情. 


 $ cpan CGI::Prototype HTTP::Cookies::Safari Test::Pod 

还有一个工具：Perl综合典藏网PLUS, 是对Perl综合典藏网. pm完全的重写. 但它不是Perl核心包的一部份, 如下：
...
Perl综合典藏网PLUS模块同样有一个方便的脚本, 叫做Perl综合典藏网p. 执行起来用i开关并列出要安装的模块列表, 像如下那样：
...
分析一下这本书的国际标准书号（0596102062）. 从Perl综合典藏网里安装一下Business::ISBN模块, 并且用它来从ISBN数字中抽取国家代码和发行商代码. 
